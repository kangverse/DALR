...
from model_modified import MCSE, BertForCL, RobertaForCL, ResNetVisnModel, ClipVisnModel
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=768,
        help="The width of the vision model."
    )

    parser.add_argument(
        '--first_teacher_model_name_or_path',
        type=str,
        default='Models/simcse',
        help='The teacher model to use.',
    )
    parser.add_argument(
        '--second_teacher_model_name_or_path',
        type=str,
        default='Models/RankCSE',
        help='The teacher model to use.',
    )
    parser.add_argument(
        "--image_res",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Temperature for teacher models.",
    )
    parser.add_argument(
        "--alpha_",
        type=float,
        default=0.33,
        help="The weight for the first teacher model.",
    )
    parser.add_argument(
        "--beta_",
        type=float,
        default=1.0,
        help="The weight for the divergence.",
    )
    parser.add_argument(
        "--distillation_loss",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.50,
        help="The weight for the second teacher model.",
    )
    parser.add_argument(
        "--using_threshhold",
        action="store_true"
...
#                 intra_loss, inter_loss, kd_loss, sd_loss, loss_consistency =  model.compute_loss(batch, cal_inter=True)
#                 # intra_loss, inter_loss =  model.compute_loss(batch, cal_inter=True)
#                 print(f'intra_loss: {intra_loss}, inter_loss: {inter_loss}, kd_loss: {kd_loss}, sd_loss: {sd_loss}, loss_consistency: {loss_consistency}')
#                 loss =intra_loss + 0.4 * inter_loss + 0.01 * kd_loss + 0.01 * loss_consistency + 0.01 * sd_loss 
#                 # + 0.01 * kl_loss
#             else:
...
#                             },
#                             os.path.join(args.output_dir, 'mse.pt')
#                         )
                    
#                     # model.lang_model.save_pretrained(args.output_dir)

#                     # if args.framework.lower() == 'mse':
#                     #     torch.save(
#                     #         {
#                     #             'visn_model': model.visn_model.state_dict(),
...
#                 # model.lang_model.save_pretrained(path)
...
def main():
    ...
    train_dataset_textonly = ImgSentDataset(text_file=args.text_file, feature_file=None)
    train_dataloader_textonly = DataLoader(train_dataset_textonly, shuffle=True, batch_size=args.per_device_train_batch_size, collate_fn=data_collator)
    textonly_loader = inf_train_gen(train_dataloader_textonly)

    train_dataset_pair = ImgSentDataset(text_file=args.caption_file, feature_file=args.feature_file)
    train_dataloader_pair = DataLoader(train_dataset_pair, shuffle=True, batch_size=args.per_device_train_batch_size, collate_fn=data_collator)
    pair_loader = inf_train_gen(train_dataloader_pair)
    ...
    # Training loop
    total_batch_size = args.per_device_train_batch_size * torch.cuda.device_count() * args.gradient_accumulation_steps
    logger.info("***** Running training *****")
    logger.info(f"  Num text examples = {len(train_dataset_textonly)}")
    logger.info(f"  Num paired examples = {len(train_dataset_pair)}")
    logger.info(f"  Num Epochs = {args.num_train_epochs}")
    ...
    for epoch in range(args.num_train_epochs):
        for step in range(num_update_steps_per_epoch):
            ...
            with torch.cuda.amp.autocast():  # Mixed precision training
                if step % paired_sample_step == 0 and args.framework.lower() == 'mse':
                    intra_loss, inter_loss, kd_loss, sd_loss, loss_consistency = model.compute_loss(batch, cal_inter=True)
                    loss = intra_loss + 0.4 * inter_loss + 0.2 * kd_loss + 0.1 * loss_consistency + 0.01 * sd_loss
                else:
                    loss = model.compute_loss(batch, cal_inter=False)
        ...
        eval_metric = evaluate(model, args, tokenizer)
        logger.info(f"Eval metric for epoch {epoch}: {eval_metric}")
    ...
    logger.info(f"Best eval metric: {best_metric}")
